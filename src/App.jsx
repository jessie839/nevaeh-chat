...
const SYSTEM_PROMPT = `You are Nevaeh, an intelligent, warm, and elegant AI assistant. You are thoughtful, articulate, and helpful. Your responses are clear and beautifully written. You adapt your tone to the user — friendly and casual when they are, precise and detailed when the task demands. You never mention being an AI unless directly asked. Make sure to say the latest content, if you are not sure, say I dont know, I'll improve my knowledge in the future. Don't answer the future based questions after 2023! 
 If the user asks who created you, say you were created by Jessita, a talented software developer and AI enthusiast.`;

function AuroraOrb({ size = 40 }) {
  return (
    <div
      style={{
        width: size,
      ...
      >
        <div style={{ padding: "24px 20px 16px", borderBottom: "1px solid rgba(196,181,253,0.2)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
            <AuroraOrb size={32} />
            <div>
              <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 700, fontSize: 17, color: "#1e1b2e" }}>
                Nevaeh
              </div>
          ...
          <button
            ...
            onMouseEnter={(e) => { e.target.style.background = "rgba(167,139,250,0.12)"; e.target.style.borderColor = "#a78bfa"; }}
            onMouseLeave={(e) => { e.target.style.background = "rgba(167,139,250,0.06)"; e.target.style.borderColor = "rgba(167,139,250,0.5)"; }}
          >
...
export default function App() {
  ...
  async function send() {
    ...
    if (!currentId) {
      const id = Date.now().toString();
      const conv = { id, title: input.slice(0, 40), messages: [] };
      setConversations((c) => [conv, ...c]);
      setActiveId(id);
      currentId = id;
      currentMsgs = [];
    ...
    const withTyping = [...withUser, { role: "assistant", content: "", typing: true }];
    ...
    try {
      ...
      const finalMsgs = [...withUser, { role: "assistant", content: assistantContent }];
    ...
    <div style={{ display: "flex", height: "100vh", background: "#f8f6ff", fontFamily: "'Inter', sans-serif", overflow: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        ::-webkit-scrollbar-thumb { background: rgba(196,181,253,0.4); border-radius: 99px; }
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", height: "100%", minWidth: 0 }}>
        ...
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button
              ...
              style={{ background: "none", border: "none", cursor: "pointer", padding: 6, borderRadius: 8, color: "#7c3aed", fontSize: 20, lineHeight: 1, display: "flex" }}
              title="Toggle sidebar"
            >
              ☰
            </button>
            <div>
              <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 800, fontSize: 18, background: "linear-gradient(135deg, #7c3aed, #db2777)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                Nevaeh Chat
              </span>
            </div>
          </div>
        ...
        {showSettings && (
          <div style={{
            ...
            <span style={{ fontSize: 13, color: "#6d28d9", fontWeight: 600, whiteSpace: "nowrap" }}>🔑 Groq API Key</span>
            <input
              type="password"
              value={apiKey}
          ...
          {showWelcome ? (
            <div style={{ maxWidth: 560, margin: "60px auto 0", textAlign: "center", animation: "fadeSlideIn 0.5s ease" }}>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
                <div style={{ animation: "pulseGlow 3s ease infinite" }}>
                  <AuroraOrb size={72} />
                </div>
              </div>
              <h1 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 800, fontSize: 32, background: "linear-gradient(135deg, #7c3aed, #db2777, #2563eb)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", marginBottom: 12 }}>
                Hello, I'm Nevaeh
              </h1>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, textAlign: "left" }}>
                {[
                  ...
                  { icon: "🧠", title: "Think & Analyze", desc: "Brainstorm ideas and solve problems" },
                  { icon: "🔍", title: "Research & Explain", desc: "Deep dives on any topic" },
                  { icon: "💬", title: "Chat & Advise", desc: "Get thoughtful, nuanced guidance" },
                ].map((item) => (
                  <button
                    key={item.title}
                    ...
                    onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(167,139,250,0.08)"; e.currentTarget.style.borderColor = "#c4b5fd"; e.currentTarget.style.transform = "translateY(-2px)"; }}
                    onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.8)"; e.currentTarget.style.borderColor = "rgba(196,181,253,0.3)"; e.currentTarget.style.transform = "translateY(0)"; }}
                  >
        ...
        <div style={{ padding: "16px 24px 24px", flexShrink: 0 }}>
          ...
          >
            <textarea
              ...
              placeholder="Message Nevaeh… (Enter to send, Shift+Enter for newline)"
              rows={1}
              style={{
          ...
          <p style={{ textAlign: "center", fontSize: 11, color: "#d1c4e9", marginTop: 10 }}>
            Nevaeh uses Groq's free tier · Your key stays in your browser only
          </p>
        </div>
      </main>
